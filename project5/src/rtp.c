...
packet_t *packetize(char *buffer, int length, int *count) {
    ...
    int pack = length / MAX_PAYLOAD_LENGTH; 
    if (length % MAX_PAYLOAD_LENGTH != 0) {
        pack++; 
    }

    packets = malloc((size_t) pack * sizeof(packet_t)); 
    *count = pack;
    ...
    for (int i = 0; i < pack; i++) {
        ...
        memcpy(packets[i].payload, (i * MAX_PAYLOAD_LENGTH) + buffer, (long unsigned int) packets[i].payload_length); //copy payload length characters into that index

        packets[i].checksum = checksum(packets[i].payload, packets[i].payload_length); //compute the checksum value based on data in buffer
    }
    ...
    do {
        ...
        do {
            ...
            */
            ...
            if ((packet.type == DATA || packet.type == LAST_DATA) && checksum(packet.payload, packet.payload_length) == packet.checksum) { //if checksum's are equal
                
                buffer = realloc(buffer, (long unsigned int) buffer_length);
                buffer_length += packet.payload_length;
                
                if (buffer == NULL) {
                    return NULL; //since we malloced have to check for null
                }
                packet_t pos; //create new packet for ack
                pos.type = ACK;
                pos.payload_length = pos.checksum = 0; 

                strcat(buffer, packet.payload); //add the packet to the buffer
                net_send_packet(connection->net_connection_handle, &pos);

            } else if (packet.type == DATA || packet.type == LAST_DATA) {
                packet_t neg; //create new packet for nack
                neg.type = NACK;
                neg.payload_length = neg.checksum = 0;
                
                net_send_packet(connection->net_connection_handle, &neg); 
                packet.type = DATA;
            }
            ...
            */
            ...
            if (packet.type == ACK) {
                ...
                pthread_cond_signal(&connection->ack_cond); //send signal that ACK has been recieved
                pthread_mutex_unlock(&(connection->ack_mutex));

            } else if (packet.type == NACK) {
                connection->sig = -1;
                pthread_mutex_lock(&(connection->ack_mutex));
                pthread_cond_signal(&connection->ack_cond); //send signal that NACK has been recieved
                pthread_mutex_unlock(&(connection->ack_mutex));
            }
        ...
        if (connection->alive == 1) {
            ...
            */
            ...
            for (int i = 0; i < buffer_length; i++) {
                message->buffer[i] = buffer[i]; //since its a string, copy one to one
            }
...
static void *rtp_send_thread(void *void_ptr) {
    ...
    do {
        ...
        for (i = 0; i < array_length; i++) {
             ...
             */
            while (connection->sig == 0) {
                pthread_cond_wait(&(connection->ack_cond), &(connection->ack_mutex));
            }
            if (connection->sig == -1) { 
                i = i - 1; //resend the last packet
            }

            connection->sig = 0; //reset it
        }
